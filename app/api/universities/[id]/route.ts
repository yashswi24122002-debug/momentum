...
export async function PATCH(
  ...
  const { data, error } = await supabase.from("universities").update(updates).eq("id", id).eq("user_id", user.id).select().single();
...
export async function DELETE(
  ...
  const { id } = await params;
  const { error } = await supabase.from("universities").delete().eq("id", id).eq("user_id", user.id);
