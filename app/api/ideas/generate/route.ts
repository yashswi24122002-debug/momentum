...
export async function POST() {
  ...
  try {
    ...
    await checkAndIncrementUsage(supabase, user.id, "ideas_generate", isAdmin, IDEA_COUNT);
  } catch (error) {
  ...
  try {
    ideas = await generateContentAsUser(user.id, isAdmin, buildPrompt(signals), IdeasResponseSchema);
  } catch (error) {
